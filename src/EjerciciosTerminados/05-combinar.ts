...
(() => {
    ...
    // combineLatest emite el conjunto de los ultimos valores emitidos por los observables que recibe
    combineLatest(letras$, numeros$).pipe(
        map(([a, b]) => a + b) // mediante la destructuración tomo los valores de los arreglos
    ).subscribe(console.log)
