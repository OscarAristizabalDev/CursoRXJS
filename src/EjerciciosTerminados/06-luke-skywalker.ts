...
(() => {
    ...
    getRequest(`${SW_API}/people/2`).pipe(
        ...
        // el operador zip pertime agrupar observables
        switchMap(resp => zip(of(resp), getRequest(resp.species[0]))),
        map(([personaje, especie]) => ({ personaje, especie })) // se realiza la destructuración del arreglo para devolverle en variables
